    ...
    <div className="flex flex-col items-center justify-between w-full max-w-3xl mx-auto h-full">
      ...
      <div className="w-full text-center mb-8 mt-6">
        <h1 className="text-3xl md:text-4xl font-bold text-blue-300">
          {interviewTitle} <span className="inline-block bg-blue-100 text-blue-600 px-3 py-2 rounded-lg mt-2">prepare for?</span>
        </h1>
      </div>
      ...
      <div className="flex flex-col items-center mb-8">
        <div className="p-3 bg-gray-100 rounded-full mb-2">
          {isListening ? <Mic className="h-6 w-6 text-blue-500" /> : <MicOff className="h-6 w-6 text-gray-400" />}
        </div>
        <p className="text-gray-500 text-sm">{isSpeaking ? "Speaking..." : isListening ? "Listening..." : "Idle"}</p>
      </div>

      {/* Last Message Transcript */}
      {lastMessage && (
        <div className="w-full max-w-lg bg-white rounded-lg p-4 mb-8 shadow-sm">
          <p className="text-gray-700 text-center animate-fadeIn">{lastMessage}</p>
        </div>
      )}
        ...
        {callStatus !== CallStatus.ACTIVE ? (
          <button
            className={cn(
              ...
              callStatus === CallStatus.CONNECTING ? "bg-yellow-500 animate-pulse" : "bg-green-500 hover:bg-green-600",
            )}
            onClick={handleCall}
